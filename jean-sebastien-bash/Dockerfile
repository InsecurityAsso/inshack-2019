...
RUN echo '* * * * * root docker kill $(docker ps -f "name=jean-sebastien-bash-sandbox" -f "status=running" | grep -P "Up \d+ minutes" | cut -d" " -f 1)' >> /etc/crontab && \
    echo >> /etc/crontab
...
CMD export HOME=/home/user && service docker start && \
    docker login -u ${DOCKER_USER} -p ${DOCKER_PASSWORD} registry-chal.infra.insecurity-insa.fr && \
